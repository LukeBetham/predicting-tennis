# Predicting Tennis - Modelling Outcomes with Machine Learning
...
## The Data
For my project I will mainly be using data extracted from a docker container (https://github.com/mcekovic/tennis-crystal-ball/issues/337) - which was created on the back of a large open source ATP data repository from Github = https://github.com/JeffSackmann/tennis_atp

I checked that this data was valid by cross referencing a number of data points between this database and the official ATP website. I will be using the last 30 years of data, as this is when the ATP started recording detailed match data, and so before this the information isn't as good. See below for visual representation of the usable matches. I only used data from 1991 as this is when the detailed data started.
...
## EDA
Before starting on the modelling some analysis and visualisation of the data to give an idea of what feature engineering would be useful. Have included a few examples of variables I looked at for example.

### Age
Age looked like it would be an important determinant as generally the win ratio peaks in the late 20's. This led me to create some features around age and experience (amount of years since first professional match).

![](images/age.png)

### Game style over time
I wanted to see if the play-style of winners changed over time - for instance below you can see that the number of aces have increased over time. I created a lot of features around strength of 1st and 2nd serve, strength of return and other play style statistics, and since I was not looking at time-series in this project I wanted to make sure that the model would generalise well to the whole time period (which based on the below chart isn't necessarily the case).
...
- 32 previous season features: Surface wins and losses, serve, return and pressure points (tie-breaks) from the previous season
...
### Modelling issues
 ...
 - Data Quality: Some of the features I was using had missing data
 	- Match stats: tried both imputing 0 and removing data for modelling (28.7%)
 	- Rank: imputed lowest possible rank (2%)
 	- Age: Removed missing age (0.3%)
 	- Height: imputed average (12.5%)
...
One of the main challenges of this project was that I wanted the prediction of the model to be the same no matter which way you input the players (i.e. player1 v.s. player2 = player2 v.s. player1, all else equal). This presents a challenge since some models are not built to allow for this. Decision trees, for instance, are not symmetric, and the classification could be completely different depending who is in the player1 spot. In order to deal with this issue, I worked on a few different model types:
...
Running the initial logistic regression with standard parameters for my symmetric model was OK to run on my laptop, but the parametrisation gridsearch promised to take over 2 days on my poor little dual core computer. So I decided to farm out the gridsearching to a more capable AWS Machine Learning EC2 instance. See my blog post at https://elasticvignette.wordpress.com/ for more details on this. 

### Machine Learning Class for Classification
During this project, I wanted to simplify the task of running the main classification models for any given dataset since I was constantly adding features and wanting to re-run all models to see how they fared. In order to help speed up this process I created a class which given a set of features and a target will run preprocessing steps (standardisation and train-test split) and then simply run a large selection of the scikit learn classification models. In addition to this I added time saving functions to run gridsearches, confusion matrices and ROC curves. The code is in this project, but for a clean version of this class and a demo on how it works, please see my separate repo on this: https://github.com/LukeBetham/machine-learning-classes

### Principal Component Analysis (PCA)
As mentioned above, some of my gridsearches were taking quite a long time to run and so I looked to reduce dimensionality. I thought this might be effective since I did have some collinearity in my features as I mentioned above. This reduction would mean that I could speed up the model's running time without losing much accuracy. This did turn out to be true, and I was able to reduce my features from 60 to 40. See below a plot of the CV score of the model as I reduce the dimensions on the features. The drop off of the mean CV score below 40 is apparent.

![](images/PCA.png)

### Modelling Results
- True Baseline: 50%
- Target Baseline (choosing the rank favourite): 63.9%
- Best Model CV Score: 67.95%

Based on this, I was able to improve dramatically on the true baseline and substantially on the target baseline. In addition I ran the model 1000 times with re-sampled observations and got the below spread of CV scores, which returned a single value t-test value of the model versus target baseline of 0, meaning that we can reject the null hypothesis that the model score has a similar mean to the baseline score. See visual representation of the models run below (purple vertical lines 99% confidence and green vertical line is baseline).

![](images/Frequentist.png)

## Comparison to Betting odds
As part of my project I decided to compare my best model to the bookmaker odds and look at potential return on investments (ROI's) for different betting strategies around the final model. I was not able to beat the accuracy of the bookmakers as the bookmakers choose the correct favourite 70.2% of the time, so about 2.2% more accurate than my model. However I wanted to see what this would mean in terms of ROI given different betting methods. See below a run-down of all the strategies and ROIs I tested:
...
- Tightest Odds: -0.8% ROI. Betting on the bookies favourite each time will reduce the losses substantially. 
...
## Further Analysis
Although the model was relatively successful in predicting above baseline, I think there is more analysis which I would like to add to the project model which I will look to in the future in order to improve further. The main ways I would like to too to improve the model are:
- Create Improved Features:
	- Fatigue - amount of hours/matches played recently.
	- Networks - integrate networks fully into model.
	- Recent matches - a weighted approach to the model with recent matches having a greater weight than previous seasons or all time.
	- Time Series - try and incorporate the change in game styles over time using time series analysis.
	- 2 versus 3 set match - in this analysis I didn't take into account what match type it was, which I think is important.
	- Bookies odds - if the goal is to beat the bookies, there is no reason the pre-match odds can't be integrated into the models as it will help align and beat the bookies (the bookmaker becomes the new baseline).
	- Clustering - Use clustering to find similar players and player types.
- Improved Models:
	- Finely tune a symmetrical Sigmoid Activation Neural Network
	- Run a linear regression to predict specific scores
	- Use bagging methods to improve the score
	- Time Series (SARIMAX)
- WTA Data: Try and recreate the same analysis with women's tennis to see if there are any differences or anything more that can be learnt from this dataset. It would be interesting to see if they could be combined or if the coefficients would differ. 
- Test the betting strategy: It would be interesting to then test the successful betting strategy live as a real test set, however this part of the project has been interrupted as the sport has been put on hold for COVID19. 
